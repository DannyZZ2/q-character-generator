...
description: "Create a consistent Q-version cartoon character package from one uploaded reference image while preserving the reference character's original body shape and head-to-body ratio as much as possible. Use when the user asks for Q版形象生成, Q-version character generation, a chibi three-view sheet, or a consistent sticker pack: generate a source-proportion three-view character sheet, generate 8 independent reaction stickers from that sheet, write a character consistency anchor Markdown file, verify the stickers against the anchor, ask the user for confirmation, then package the approved stickers and Markdown files."
---

# Q版形象生成

Use this skill when the user uploads one character reference image and wants a reusable Q-version cartoon character workflow: source-proportion three-view design, 8 independent stickers, a character anchor Markdown file, consistency verification, user confirmation, and final packaging.
...
- Preserve the reference character's original body shape and head-to-body ratio as much as possible. If the reference is already 3-heads-tall, keep that ratio; otherwise do not force the character into 3-heads-tall proportions.
...
2. Identify stable character traits:
   ...
   - accessories
   - color palette
   - body shape and head-to-body ratio
   - age category and general identity
3. Generate one source-proportion Q-version cartoon three-view sheet.
...
Asset type: character design sheet / Q-version cartoon three-view turnaround with source-matched proportions
Primary request: Based on the uploaded reference image, create a cute Q-version cartoon character that keeps the reference character's original body shape and head-to-body ratio as much as possible, shown as a clean three-view model sheet.
Input images: Uploaded image is the character identity, outfit, body shape, and head-to-body ratio reference. Preserve the most recognizable facial, hairstyle/headwear, clothing, accessory, body proportion, and color cues.
Subject: one stylized cartoon version of the referenced character, cute Q-version styling with source-matched head-to-body ratio, friendly readable expression.
...
Constraints: no text labels, no watermark, no logos, no extra characters, no props unless present in the reference and important to identity. Keep all three views consistent in outfit, proportions, hairstyle/headwear, accessories, and character identity. Strongly preserve the reference character's body shape and head-to-body ratio; do not force the design into 3-heads-tall or 5-heads-tall proportions unless the reference already matches that ratio. Do not make the character realistic.
```

If the generated sheet does not preserve the reference character's body shape/head-to-body ratio or the three views are inconsistent, regenerate once with stricter proportion and consistency language before continuing.
...
- Clean dark outline.
- Soft cel shading.
- Q-version proportions that preserve the original body shape and head-to-body ratio as much as possible.
...
- Keep the original body shape and head-to-body ratio from the reference image and `00-three-view.png` as much as possible.
- Do not switch to a different head-to-body ratio, realistic adult anatomy, 3D toy rendering, flat vector redesign, or photorealism.
...
- Same body shape and head-to-body ratio.
...
Subject: same Q-version cartoon character described in `character-anchor.md`; preserve headwear/hairstyle, face shape, eye style, outfit, accessories, body shape, head-to-body ratio, color palette, and polished 2D anime mascot style.
Pose/expression: <emotion/action-specific instruction>.
Composition/framing: single standalone sticker centered on a clean light background, generous padding, full body if it preserves the source-matched proportions; otherwise upper body is acceptable when it better preserves identity.
...
Constraints: no text, no watermark, no logo, no extra character, no unrelated props, no scene background. Do not change outfit, headwear/hairstyle, accessories, body shape, head-to-body ratio, or color palette. Do not force the character into 3-heads-tall or 5-heads-tall proportions unless the reference already matches that ratio. Do not make it realistic, 3D, or flat vector.
```
...
- Same body shape and head-to-body ratio: pass/fail.
